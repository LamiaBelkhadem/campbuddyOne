...
const upsertProfile = async (req, res) => {
    ...
    if (user.profile) {
        const updatedProfile = await Profile.findByIdAndUpdate(user.profile._id, {
            $set: profile
        }, {new: true}).lean();
...
const remove = async (req, res, next) => {
    ...
    try {
        ...
        return res.status(200).json(messageResponse("Profile has been deleted"));
    } catch (err) {
        next(err);
    }
};
...
const myProfile = async (req, res) => {
    ...
    if (!myProfile) return res.json(errorMessage("Profile not found, please create it "));
    return res.json({profile: myProfileObj})
}

export const profile = {
    upsertProfile, getOne, remove, getAll, myProfile
};
